...
class InterHeteroDataset(Dataset):
    ...
    def _GetitemSingleCar(
        self, cavData: CAVData, modality_name, ego_pose: np.ndarray[np.float64], ego_pose_clean: np.ndarray[np.float64]
    ):
        """
    ...
    def __getitem__(self, idx):
        ...
        legalCAVData = base_data_dict[ego_id]
        for modality_name in ["m1", "m2"]:
            sensor_type = self.sensor_type_dict[modality_name]
            ...
            selected_cav_processed = self._GetitemSingleCar(legalCAVData, modality_name, ego_pose, ego_pose_clean)
