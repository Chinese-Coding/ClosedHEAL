        ...
        def get_item_test(self, base_data_dict, idx):
            ...
            for cav_id in cav_id_list:
                ...
                selected_cav_processed.update(
                    {"transformation_matrix": transformation_matrix, "transformation_matrix_clean": transformation_matrix_clean}
                )
            ...
            for modality_name in self.modality_name_list:
                ...
                if self.sensor_type_dict[modality_name] == "lidar":
                    processed_lidar_torch_dict = eval(f"self.pre_processor_{modality_name}").collate_batch(
                        eval(f"inputs_list_{modality_name}")
                    )
        ...
        def collate_batch_test(self, batch):
            ...
            for cav_id, cav_content in batch.items():
                ...
                if sensor_type == "lidar":
                    ...
                    processed_lidar_torch_dict = eval(f"self.pre_processor_{modality_name}").collate_batch(
                        [cav_content[f"processed_features_{modality_name}"]]
                    )
                ...
                transformation_matrix_clean_torch = torch.from_numpy(
