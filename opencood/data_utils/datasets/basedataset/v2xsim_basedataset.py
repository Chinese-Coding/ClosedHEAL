    ...
    def reinitialize(self):
        ...
        for i, scene_info in enumerate(self.dataset_info_pkl):
            ...
            for j, cav_id in enumerate(cav_ids):
                ...
                self.scene_database[i][cav_id]["lidar"] = self.scene_database[i][cav_id]["lidar"].replace(
                    "/GPFS/rhome/yifanlu/workspace/dataset/v2xsim2-complete", "dataset/V2X-Sim-2.0"
                )

                self.scene_database[i][cav_id]["params"] = OrderedDict()
                self.scene_database[i][cav_id]["params"]["lidar_pose"] = tfm_to_pose(
                ...
                self.scene_database[i][cav_id]["params"]["object_ids"] = scene_info[f"labels_{cav_id}"][
                    "gt_object_ids"
                ].tolist()
