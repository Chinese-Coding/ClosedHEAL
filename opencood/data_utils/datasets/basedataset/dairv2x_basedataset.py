...
class DAIRV2XBaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx):
        ...
        if self.load_camera_file:
            ...
            data[1]["params"]["camera0"]["extrinsic"] = rot_and_trans_to_trasnformation_matrix(
                read_json(
                    os.path.join(
                        self.root_dir, "infrastructure-side/calib/virtuallidar_to_camera/" + str(inf_frame_id) + ".json"
                    )
                )
            )
        ...
        if getattr(self, "heterogeneous", False):
            ...
            if self.train:  # randomly choose RSU or Veh to be Ego
                ...
                if (
                    self.adaptor.mapping_dict[data[0]["modality_name"]] not in self.ego_modality
                    and self.adaptor.mapping_dict[data[1]["modality_name"]] in self.ego_modality
                ):
                    data[0], data[1] = data[1], data[0]
                    data[0]["ego"] = True
                    data[1]["ego"] = False
    ...
    def generate_object_center_single(self, cav_contents, reference_lidar_pose, **kwargs):
        ...
        for cav_content in cav_contents:
            cav_content["params"]["vehicles_single"] = (
                ...
                if self.label_type == "camera"
                else cav_content["params"]["vehicles_single_all"]
            )
    ...
    def generate_object_center_single_hetero(self, cav_contents, reference_lidar_pose, modality):
        """
