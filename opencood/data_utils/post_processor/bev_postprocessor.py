...
class BevPostprocessor(BasePostprocessor):
    ...
    def update_label_map(self, label_map, bev_corners, reg_targets):
        ...
        points = np.concatenate([xx.reshape(-1, 1), yy.reshape(-1, 1)], axis=-1)  # pixel
    ...
    def reg_map_to_bbx_corners(self, reg_map, mask):
        ...
        grid_x = torch.arange(
            self.geometry_param["L1"], self.geometry_param["L2"], grid_size, dtype=torch.float32, device=device
        )
        grid_y = torch.arange(
            self.geometry_param["W1"], self.geometry_param["W2"], grid_size, dtype=torch.float32, device=device
        )
