...
class PointPillarUncertaintyLoss(PointPillarLoss):
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        reg_preds_w_angle, reg_targets_w_angle = self.add_sin_difference_and_angle(
            reg_preds, reg_targets
        )  # note the difference
        reg_loss = weighted_smooth_l1_loss(
            reg_preds_w_angle[..., :7], reg_targets_w_angle[..., :7], weights=reg_weights, sigma=self.reg["sigma"]
        )
        ...
        unc_loss = self.unc_loss_func(reg_preds_w_angle, reg_targets_w_angle, unc_preds, reg_weights)
        ...
        if self.dir:
            ...
            dir_logits = (
                output_dict[f"dir_preds{suffix}"].permute(0, 2, 3, 1).contiguous().view(batch_size, -1, 2)
            )  # [N, H*W*#anchor, 2]
        ...
        if self.iou:
            ...
            boxes3d_pred = VoxelPostprocessor.delta_to_boxes3d(
                output_dict[f"reg_preds{suffix}"].permute(0, 2, 3, 1).contiguous().detach(), output_dict["anchor_box"]
            )[pos_pred_mask]
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None):
        ...
        print(
            ...
            % (epoch, batch_id + 1, batch_len, total_loss, cls_loss, reg_loss, dir_loss, iou_loss, unc_loss)
        )
        ...
        if not writer is None:
            ...
            writer.add_scalar("Iou_loss", iou_loss, epoch * batch_len + batch_id)
            writer.add_scalar("Unc_loss", unc_loss, epoch * batch_len + batch_id)
    ...
    def add_sin_difference_and_angle(boxes1, boxes2, dim=6):
        ...
        boxes1_w_angle = torch.cat(
            [boxes1[..., :dim], rad_pred_encoding, boxes1[..., dim:]], dim=-1
        )  # originally, boxes1[..., dim + 1:]], dim=-1)
        boxes2_w_angle = torch.cat(
            [boxes2[..., :dim], rad_tg_encoding, boxes2[..., dim:]], dim=-1
        )  # originally, boxes1[..., dim + 1:]], dim=-1)
