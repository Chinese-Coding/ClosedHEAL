    ...
    def forward(self, output_dict, label_dict):
        ...
        if self.iou is not None:
            ...
            boxes3d_pred = VoxelPostprocessor.delta_to_boxes3d(
                preds_dict["reg_preds"].permute(0, 2, 3, 1).contiguous().detach(), output_dict["anchor_box"]
            )[pos_pred_mask]
            ...
            iou_pos_targets = (
                aligned_boxes_iou3d_gpu(
                    boxes3d_pred.float()[:, [0, 1, 2, 5, 4, 3, 6]], boxes3d_tgt.float()[:, [0, 1, 2, 5, 4, 3, 6]]
                )
                .detach()
                .squeeze()
            )
            ...
            self.loss_dict.update({"iou_loss": iou_loss_reduced})
