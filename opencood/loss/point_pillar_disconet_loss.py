...
class PointPillarDiscoNetLoss(PointPillarLoss):
    ...
    def forward(self, output_dict, target_dict):
        ...
        kd_loss_feature = kl_loss_mean(F.log_softmax(student_feature, dim=1), F.softmax(teacher_feature, dim=1))
        ...
        total_loss += kd_loss
        self.loss_dict.update({"total_loss": total_loss.item(), "kd_loss": kd_loss.item()})
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None):
        ...
        print(
            ...
            % (epoch, batch_id + 1, batch_len, total_loss, cls_loss, reg_loss, dir_loss, iou_loss, kd_loss)
        )
