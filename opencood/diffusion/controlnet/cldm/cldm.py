...
# TODO: 这个类原本就有, 作者只是稍作修改
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, per_layers=False, **kwargs):
        hs, houts = [], []
        ...
        if control is not None:
            h += control.pop()
        # TODO: 这个 `output_backs` 里面为什么就没有 `torch.no_grad` 呢?
...
class ControlLDM(LatentDiffusion):
    ...
    def get_unconditional_conditioning(self, N):
        return self.get_learned_conditioning(c=[""] * N)
