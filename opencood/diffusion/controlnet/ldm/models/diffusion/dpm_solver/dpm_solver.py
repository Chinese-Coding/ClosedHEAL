...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        Update: We support discrete-time diffusion checkpoints by implementing a picewise linear interpolation for log_alpha_t.
                We recommend to use schedule='discrete' for the discrete-time diffusion checkpoints, especially for high-resolution images.
        ***
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == "discrete":
            return interpolate_fn(t.reshape((-1, 1)), self.t_array.to(t.device), self.log_alpha_array.to(t.device)).reshape(
                (-1)
            )
...
def model_wrapper(
    ...
    We support four types of the diffusion model by setting `model_type`:
        ...
        3. "v": velocity prediction model. (Trained by predicting the velocity).
            ...
            [1] Salimans, Tim, and Jonathan Ho. "Progressive distillation for fast sampling of diffusion checkpoints."
    ...
    We support three types of guided sampling by DPMs by setting `guidance_type`:
        ...
        2. "classifier": classifier guidance sampling [3] by DPMs and another classifier.
            ...
            ``
            [3] P. Dhariwal and A. Q. Nichol, "Diffusion checkpoints beat GANs on image synthesis,"
...
class DPM_Solver:
    def __init__(self, model_fn, noise_schedule, predict_x0=False, thresholding=False, max_val=1.0):
        ...
        [1] Chitwan Saharia, William Chan, Saurabh Saxena, Lala Li, Jay Whang, Emily Denton, Seyed Kamyar Seyed Ghasemipour, Burcu Karagol Ayan, S Sara Mahdavi, Rapha Gontijo Lopes, et al. Photorealistic text-to-image diffusion checkpoints with deep language understanding. arXiv preprint arXiv:2205.11487, 2022b.
        """
        self.model = model_fn
    ...
    def singlestep_dpm_solver_second_update(
        ...
        if self.predict_x0:
            ...
            if model_s is None:
                model_s = self.model_fn(x, s)
            x_s1 = expand_dims(torch.exp(log_alpha_s1 - log_alpha_s), dims) * x - expand_dims(sigma_s1 * phi_11, dims) * model_s
            model_s1 = self.model_fn(x_s1, s1)
            if solver_type == "dpm_solver":
                x_t = (
    ...
    def dpm_solver_adaptive(
        ...
        [1] A. Jolicoeur-Martineau, K. Li, R. Piché-Taillefer, T. Kachman, and I. Mitliagkas, "Gotta go fast when generating data with score-based checkpoints," arXiv preprint arXiv:2105.14080, 2021.
        """
        ns = self.noise_schedule
    ...
    def sample(
        ...
        Args:
            ...
            denoise_to_zero: A `bool`. Whether to denoise to time 0 at the final step.
                ...
                for diffusion checkpoints sampling by diffusion SDEs for low-resolutional images
