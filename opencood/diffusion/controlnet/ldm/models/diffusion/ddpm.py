...
class DDPM(pl.LightningModule):
    ...
    def q_sample(self, x_start, t, noise=None):  # TODO: 调用的是这个
        noise = default(noise, lambda: torch.randn_like(x_start))  # 随机生成一个和输入图片一模一样的噪声
        return (
            extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start
            + extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise
        )  # 这段代码的核心在于将初始输入 x_start 和噪声 noise 按照特定的时间步 t 进行加权组合，从而生成扩散过程中的样本
...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):  # TODO: 调用的是这个
    ...
    def get_learned_conditioning(self, c=None, image=None):  # 调用的是这个
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "encode") and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(text=c, image=image)
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()
            else:
                c = self.cond_stage_model(text=c, image=image)
        else:
            assert hasattr(self.cond_stage_model, self.cond_stage_forward)
            c = getattr(self.cond_stage_model, self.cond_stage_forward)(text=c, image=image)
        return c
...
class LatentInpaintDiffusion(LatentFinetuneDiffusion):
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False):
        ...
        z, c, x, xrec, xc = super().get_input(
            batch, self.first_stage_key, return_first_stage_outputs=True, force_c_encode=True, return_original_cond=True, bs=bs
        )
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False):
        ...
        for ck in self.concat_keys:
            ...
            if exists(self.reshuffle_patch_size):
                ...
                cc = rearrange(
                    cc, "b c (p1 h) (p2 w) -> b (p1 p2 c) h w", p1=self.reshuffle_patch_size, p2=self.reshuffle_patch_size
                )
            if bs is not None:
                cc = cc[:bs]
                cc = cc.to(self.device)
