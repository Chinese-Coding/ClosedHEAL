...
class control_extractor(Null_model):
    def __init__(
        ...
        seed=-1,
        t=150,
        basic="./tools/controlnet/checkpoints",
        yaml="control_v11f1p_sd15_depth.yaml",
        sd_ckpt="v1-5-pruned.ckpt",
        cn_ckpt="control_v11f1p_sd15_depth.pth",
        prompt="a photo of a room and furniture",  # 房间和家具的照片
    ) -> None:
        super().__init__()
        if load_model:
            self.prompt = prompt
            self.capturer = capture(seed=seed, basic=basic, yaml=yaml, sd_ckpt=sd_ckpt, cn_ckpt=cn_ckpt, t=t)
