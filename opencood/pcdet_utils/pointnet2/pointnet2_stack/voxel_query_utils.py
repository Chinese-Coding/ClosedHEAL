...
class VoxelQuery(Function):
    ...
    def forward(
        ...
        pointnet2.voxel_query_wrapper(
            M, Z, Y, X, nsample, radius, z_range, y_range, x_range, new_xyz, xyz, new_coords, point_indices, idx
        )
...
class VoxelQueryAndGrouping(nn.Module):
    ...
    def forward(
        ...
        assert new_coords.shape[0] == new_xyz_batch_cnt.sum(), "new_coords: %s, new_xyz_batch_cnt: %s" % (
