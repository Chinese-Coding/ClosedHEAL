...
class MergeDuplicate(SparseModule):
    ...
    def forward(self, x: spconv.SparseConvTensor):
        ...
        res = spconv.SparseConvTensor(out_feature, out_indices, x.spatial_shape, x.batch_size, x.grid)
        return res
...
def fuseSparseTensor(x_list):
    ...
    res = spconv.SparseConvTensor(new_features, new_indice, x_list[0].spatial_shape, x_list[0].batch_size, x_list[0].grid)
    return res
...
class warpSparseTensor(SparseModule):
    ...
    def construct_new_tensor(self, x, points_xyz, voxel_size, range3d):
        ...
        """
        mask = (
            (points_xyz[:, 0] > range3d[0])
            & (points_xyz[:, 0] < range3d[3])
            & (points_xyz[:, 1] > range3d[1])
            & (points_xyz[:, 1] < range3d[4])
            & (points_xyz[:, 2] > range3d[2])
            & (points_xyz[:, 2] < range3d[5])
        )
