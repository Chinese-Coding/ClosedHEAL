...
def logbessel_I_scipy(nu, z, check=True):
    ...
    if check:
        assert len(result[result == 0]) == 0, "Bessel functions take " + "value 0 for z = {}".format(z[result == 0])
...
def logbessel_I_asymptotic(nu, z):
    ...
    result[z > nu] = z[z > nu] - 0.5 * torch.log(z[z > nu]) - 0.5 * torch.log(torch.tensor(2 * pi))
    return result
...
def ub_Ak(nu, z):
    ...
    ub[:, nu.reshape(-1) >= 0.5] = torch.min(B(0, nu[nu >= 0.5], z), B_tilde(2, nu[nu >= 0.5], z))
...
def logbessel_I(nu, z, fast=False, check=True):
    ...
    if check:
        ...
        # logbessel_approx
        assert len(result[torch.isnan(result)]) == 0, "Bessel functions take " + "NaN value for z = {}".format(
            z[torch.isnan(result)]
        )
        assert len(result[torch.isinf(result)]) == 0, "Bessel functions take " + "inf value for z = {}".format(
