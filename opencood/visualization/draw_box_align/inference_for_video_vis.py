...
def test_parser():
    ...
    parser.add_argument(
        "--use_laplace", action="store_true", help="whether to use laplace to simulate noise. Otherwise Gaussian"
    )
    parser.add_argument(
        "--fusion_method", type=str, default="intermediate", help="no, no_w_uncertainty, late, late_w_ba, early or intermediate"
    )
    parser.add_argument("--save_vis_interval", type=int, default=1, help="save how many numbers of visualization result?")
            ...
            for i, batch_data in enumerate(data_loader):
                ...
                with torch.no_grad():
                    ...
                    if opt.fusion_method == "late":
                        ...
                        pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_no_fusion(
                            batch_data, model, opencood_dataset
                        )
                    elif opt.fusion_method == "no_w_uncertainty":
                        pred_box_tensor, pred_score, gt_box_tensor, uncertainty_tensor = (
                            inference_utils.inference_no_fusion_w_uncertainty(batch_data, model, opencood_dataset)
                        )
                    else:
                        raise NotImplementedError("Only no, early, late and intermediate" "fusion is supported.")
