...
class SecondSSFAUncertainty(nn.Module):
    def __init__(self, args):
        ...
        self.cls_head = nn.Conv2d(self.out_channel, args["anchor_num"], kernel_size=1)
        self.reg_head = nn.Conv2d(self.out_channel, 7 * args["anchor_num"], kernel_size=1)
        self.unc_head = nn.Conv2d(self.out_channel, uncertainty_dim * args["anchor_num"], kernel_size=1)

        self.use_dir = False
        if "dir_args" in args.keys():
            self.use_dir = True
            self.dir_head = nn.Conv2d(
                self.out_channel, args["dir_args"]["num_bins"] * args["anchor_num"], kernel_size=1
            )  # BIN_NUM = 2
    ...
    def forward(self, data_dict):
        ...
        output_dict = {"cls_preds": cls_preds, "reg_preds": reg_preds, "unc_preds": unc_preds}
