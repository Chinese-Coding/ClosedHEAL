...
from opencood.models.sub_modules.lss_submodule import CamEncode, BevEncode, CamEncode_Resnet101
...
class LiftSplatShootVoxel(nn.Module):
    def __init__(self, args):
        ...
        self.cls_head = nn.Conv2d(self.bevout_feature, args["anchor_number"], kernel_size=1)
        self.reg_head = nn.Conv2d(self.bevout_feature, 7 * args["anchor_number"], kernel_size=1)
        if "dir_args" in args.keys():
            self.use_dir = True
            self.dir_head = nn.Conv2d(
                self.bevout_feature, args["dir_args"]["num_bins"] * args["anchor_number"], kernel_size=1
            )  # BIN_NUM = 2
        else:
            self.use_dir = False
    ...
    def get_geometry(self, rots, trans, intrins, post_rots, post_trans):
        ...
        points = torch.cat(
            (
                points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],  # points[:, :, :, :, :, 2:3] ranges from [4, 45) meters
                points[:, :, :, :, :, 2:3],
            ),
            5,
    ...
    def forward(self, data_dict):
        ...
        psm = self.cls_head(x)
        rm = self.reg_head(x)
        output_dict = {"psm": psm, "rm": rm, "depth_items": depth_items}
