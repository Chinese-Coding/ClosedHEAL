...
from opencood.models.sub_modules.resblock import Bottleneck
...
def weighted_fuse(x, score, record_len, affine_matrix, align_corners):
    ...
    for b in range(B):
        ...
        scores_in_ego = torch.where(
            torch.isnan(scores_in_ego), torch.zeros_like(scores_in_ego, device=scores_in_ego.device), scores_in_ego
        )
...
class PyramidFusion(ResnetBEVBackbone):
    def __init__(self, model_cfg):
        ...
        if model_cfg["resnext"]:
            ...
            self.resnet = PyramidResnet(
                Bottleneck,
                model_cfg["layer_nums"],
                model_cfg["layer_strides"],
        ...
        for i in range(self.num_levels):
            setattr(
                self,
                f"single_head_{i}",
                nn.Conv2d(model_cfg["num_filters"][i], 1, kernel_size=1),
            )
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, agent_modality_list=None, cam_crop_info=None):
        ...
        for i in range(self.num_levels):
            ...
            if crop_mask_flag and not self.training:
                ...
                for cam_modality in cam_modality_set:
                    crop_H = (
                        H / cam_crop_info[cam_modality][f"crop_ratio_H_{cam_modality}"] - 4
                    )  # There may be unstable response values at the edges.
                    crop_W = (
                        W / cam_crop_info[cam_modality][f"crop_ratio_W_{cam_modality}"] - 4
                    )  # There may be unstable response values at the edges.
