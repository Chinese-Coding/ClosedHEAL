...
class DiscoFusion(nn.Module):
    ...
    def forward(self, xx, record_len, affine_matrix):
        ...
        for b in range(B):
            ...
            t_matrix = affine_matrix[b][:N, :N, :, :]
            i = 0  # ego
            neighbor_feature = warp_affine_simple(split_x[b], t_matrix[i, :, :, :], (H, W))
            # (N, C, H, W)
...
class CoBEVT(nn.Module):
    ...
    def forward(self, x, record_len, affine_matrix):
        ...
        com_mask = repeat(
            com_mask, "b h w c l -> b (h new_h) (w new_w) c l", new_h=regroup_feature.shape[3], new_w=regroup_feature.shape[4]
        )
