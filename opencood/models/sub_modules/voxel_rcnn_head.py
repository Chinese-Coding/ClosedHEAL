...
class VoxelRCNNHead(nn.Module):
    def __init__(self, model_cfg, backbone_channels):
        ...
        self.cls_layers, pre_channel = self._make_fc_layers(pre_channel, fc_layers, output_channels=1)
        self.iou_layers, _ = self._make_fc_layers(pre_channel, fc_layers, output_channels=1)
        self.reg_layers, _ = self._make_fc_layers(pre_channel, fc_layers, output_channels=7)
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        roi_grid_coords_x = torch.div(
            (roi_grid_xyz[:, 0:1] - self.point_cloud_range[0]), self.voxel_size[0], rounding_mode="floor"
        )
        roi_grid_coords_y = torch.div(
            (roi_grid_xyz[:, 1:2] - self.point_cloud_range[1]), self.voxel_size[1], rounding_mode="floor"
        )
        roi_grid_coords_z = torch.div(
            (roi_grid_xyz[:, 2:3] - self.point_cloud_range[2]), self.voxel_size[2], rounding_mode="floor"
        )

        roi_grid_coords = torch.cat(
            [roi_grid_coords_x, roi_grid_coords_y, roi_grid_coords_z], dim=-1
        )  # 整数坐标 --> (sum(proposal)*6*6*6, 3)
        ...
        for k, src_name in enumerate(self.feature_source):
            ...
            cur_roi_grid_coords = torch.div(
                roi_grid_coords, cur_stride, rounding_mode="floor"
            )  # 计算下采样后的网格坐标 (sum(proposal)*6*6*6,3)
            cur_roi_grid_coords = torch.cat(
                [batch_idx, cur_roi_grid_coords], dim=-1
            )  # 将batch index与roi grid coord拼接 --> (sum(proposal)*6*6*6,4)
            ...
            pooled_features = pooled_features.view(
                -1, self.grid_size**3, pooled_features.shape[-1]
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        ...
        global_roi_grid_points = common_utils.rotate_points_along_z(local_roi_grid_points.clone(), rois[:, 6]).squeeze(
            dim=1
        )  # (1024, 216, 3) 前3维沿着z轴旋转
    ...
    def forward(self, batch_dict):
        ...
        pooled_features = (
            ...
            .contiguous()
            .view(batch_size_rcnn, -1, self.grid_size, self.grid_size, self.grid_size)
        )  # (BxN, C, 6, 6, 6)
    ...
    def assign_targets(self, batch_dict):
        ...
        gt_boxes = [
            b[m][:, [0, 1, 2, 5, 4, 3, 6]].float()
            for b, m in zip(batch_dict["object_bbx_center"], batch_dict["object_bbx_mask"].bool())
        ]  # hwl -> lwh order
        for rois, scores, gts in zip(pred_boxes, pred_scores, gt_boxes):  # each frame
            ...
            heading_label = (
                gt_of_rois[:, 6]
                + (torch.div(torch.abs(gt_of_rois[:, 6].min()), (2 * np.pi), rounding_mode="trunc") + 1) * 2 * np.pi
            ) % (
                2 * np.pi
            )  # 0 ~ 2pi
