...
class MatcherV2(nn.Module):
        ...
        for i, cavnum in enumerate(record_len):
            ...
            if len(cur_scores_list) == 0:
                clusters_batch.append(
                    [torch.Tensor([0.0, 0.0, 0.0, 1.6, 2.0, 4.0, 0]).to(torch.device("cuda:0")).view(1, 7)]  #
                )
    ...
    def cluster_fusion(self, clusters, scores):
        ...
        boxes_fused = [boxes_fused[sum(box_num_sample[:i]) : sum(box_num_sample[:i]) + l] for i, l in enumerate(box_num_sample)]
    ...
    def collect_voxel_feature(self, data_dict):
        ...
        for srcname in self.feature_source:
            ...
            features_fused = spconv.SparseConvTensor(
                new_features,
                new_indice,
                sp_tensor_fused_list[0].spatial_shape,
                len(data_dict["record_len"]),
                sp_tensor_fused_list[0].grid,
            )
