...
class ResNetBEVBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels=64):
        ...
        if "layer_nums" in self.model_cfg:
            ...
            assert (
                len(self.model_cfg["layer_nums"]) == len(self.model_cfg["layer_strides"]) == len(self.model_cfg["num_filters"])
            )
        ...
        self.resnet = ResNetModified(BasicBlock, layer_nums, layer_strides, num_filters, inplanes=model_cfg.get("inplanes", 64))
