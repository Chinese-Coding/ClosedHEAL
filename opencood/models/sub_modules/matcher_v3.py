...
class MatcherV3(nn.Module):
    ...
    def clustering(self, data_dict):
        ...
        for i, cavnum in enumerate(record_len):
            cur_boxes_list = data_dict["det_boxes"][
                sum(record_len[:i]) : sum(record_len[:i]) + cavnum
            ]  # within one sample, different cav
            cur_boxes_list_ego = []
            cur_agentid_list = []
            ...
            for agent_id in range(1, cavnum):
                tfm = x1_to_x2(lidar_poses[sum(record_len[:i]) + agent_id], lidar_poses[sum(record_len[:i])])
            ...
            if len(cur_scores_list) == 0:
                clusters_batch.append(
                    [torch.Tensor([0.0, 0.0, 0.0, 1.6, 2.0, 4.0, 0]).to(torch.device("cuda")).view(1, 7)]  # hwl
                )
    ...
    def cluster_fusion(self, clusters, scores, agentid, view_vector):
        ...
        for cl, sl, al, vl in zip(clusters, scores, agentid, view_vector):  # cl, sl are clusters and scores within one sample
            for c, s, a, v in zip(cl, sl, al, vl):  # one sample (cl) has many clusters (c), c,s,a correspond to one cluster.
                ...
                dirs_diff = dirs_diff * (1 - lt_pi) + (2 * np.pi - dirs_diff) * lt_pi
        ...
        boxes_fused = [
            boxes_fused[sum(box_num_sample[:i]) : sum(box_num_sample[:i]) + l] for i, l in enumerate(box_num_sample)
        ]  # shape [[num_of_cluster_in_sample1, 7], [num_of_cluster_in_sample2, 7], ...]

        scores_fused = torch.stack(scores_fused, dim=0)
        scores_fused = [
            scores_fused[sum(box_num_sample[:i]) : sum(box_num_sample[:i]) + l] for i, l in enumerate(box_num_sample)
        ]  # shape [[num_of_cluster_in_sample1,], [num_of_cluster_in_sample2,], ...]

        agentid_fused = [
            agentid_fused[sum(box_num_sample[:i]) : sum(box_num_sample[:i]) + l] for i, l in enumerate(box_num_sample)
        ]  # content [[[id1,id2], [id1, id2, id3], ... ], [sample2 content], ...]

        view_vector_fused = [
            view_vector_fused[sum(box_num_sample[:i]) : sum(box_num_sample[:i]) + l] for i, l in enumerate(box_num_sample)
        ]  # shape [[ [2,2], [3,2], ...], [sample2 content], ...]
