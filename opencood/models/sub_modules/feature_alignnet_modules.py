...
class ConvEncoder(nn.Module):
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6, expan_ratio=4, kernel_size=1, deformable=False):
        super().__init__()
        if not deformable:
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-6, kernel_size=7, deform=False):
        super().__init__()
        self.deform = deform
        if self.deform:
            from mmcv.ops import DeformConv2dPack as dconv2d
