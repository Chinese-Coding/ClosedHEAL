...
def box_alignment_relative_sample_np(
    ...
    nonempty_indices = [
        idx for (idx, corners) in enumerate(pred_corners_list) if len(corners) != 0
    ]  # if one agent detects no boxes, its corners is just [].

    pred_corners_world_list = [
        box_utils.project_box3d(pred_corners_list[i], lidar_pose_noisy_tfm[i]) for i in nonempty_indices
    ]  # [[N1, 8, 3], [N2, 8, 3],...]
    pred_box3d_list = [
        box_utils.corner_to_center(corner, order) for corner in pred_corners_list if len(corner) != 0
    ]  # [[N1, 7], [N2, 7], ...], angle in radian
    pred_box3d_world_list = [
        box_utils.corner_to_center(corner, order) for corner in pred_corners_world_list
    ]  # [[N1, 7], [N2, 7], ...], angle in radian
    pred_center_list = [np.mean(corners, axis=1) for corners in pred_corners_list if len(corners) != 0]  # [[N1,3], [N2,3], ...]

    pred_center_world_list = [pred_box3d_world[:, :3] for pred_box3d_world in pred_box3d_world_list]
    pred_yaw_world_list = [pred_box3d[:, 6] for pred_box3d in pred_box3d_world_list]
...
def box_alignment_relative_np(pred_corner3d_list, uncertainty_list, lidar_poses, record_len, **kwargs):
    """
    Args:
