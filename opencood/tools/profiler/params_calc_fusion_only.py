...
def train_parser():
    ...
    parser.add_argument("--model_dir", default="", help="Place holder")
    parser.add_argument("--input_data", type=str, required=True, help="input data pickle file.")
...
class PyramidModel(torch.nn.Module):
    ...
    def forward(self, heter_feature_2d, record_len, affine_matrix, agent_modality_list, cam_crop_info):
        fused_feature, _ = self.pyramid_backbone(
            heter_feature_2d, record_len, affine_matrix, agent_modality_list, cam_crop_info
        )
        if self.shrink_flag:
