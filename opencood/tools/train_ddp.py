...
def main():
    ...
    if opt.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[opt.gpu], find_unused_parameters=True)  # True
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if epoch % hypes["train_params"]["eval_freq"] == 0:
            ...
            if valid_ave_loss < lowest_val_loss:
                ...
                torch.save(model_without_ddp.state_dict(), os.path.join(saved_path, "net_epoch_bestval_at%d.pth" % (epoch + 1)))
    ...
    if opt.rank == 0:
        ...
        if len(bestval_model_list) > 1:
            ...
            bestval_model_epoch_list = [
                eval(x.split("/")[-1].lstrip("net_epoch_bestval_at").rstrip(".pth")) for x in bestval_model_list
            ]
