...
def test_parser():
    ...
    parser.add_argument("--range", type=str, default="204.8,102.4", help="detection range is [-204.8, +204.8, -102.4, +102.4]")
    parser.add_argument("--no_score", action="store_true", help="whether print the score of prediction")
    parser.add_argument("--use_cav", type=str, default="[1,2,3,4]", help="evaluate with real collaborator number")
    parser.add_argument(
        "--lidar_degrade", action="store_true", help="whether to degrade lidar. {m1:32, m3:16} and {m1:16, m3:16}"
    )
...
def main():
    ...
    if "heter" in hypes:
        ...
        hypes = update_dict(hypes, {"mapping_dict": {"m1": "m1", "m2": "m2", "m3": "m3", "m4": "m4"}})
        ...
        for i, batch_data in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                if opt.fusion_method == "late":
                    infer_result = inference_late_fusion_heter_in_order(batch_data, model, opencood_dataset, use_cav)
